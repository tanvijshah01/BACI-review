...
# save this as CSV for reuse 
waterbird_path <- file.path(curr_data_path, "output","Wauchope", "waterbird_families.csv")
write.csv(alphabetized_waterbird_families, waterbird_path, row.names=FALSE)
```
...
df_nums <- data.frame(
  count =  c(num_hyb, num_non_hyb, per_hyb),
  dataset = c("hybrids", "non_hybrids", "percent of hybrids in total")
)

kable(df_nums, caption = "Hybrids are a small fraction of overall dataset. Not including hybrids in BirdCounts dataset") %>%
  kable_styling()
...
df_loc <- data.frame(
  category = c( "unique abbrevs", "unique lat long pairs", "unique lats", "unique longs"),
  count = c(length(unique_abbrev), nrow(unique_lat_long), length(unique_lat), length(unique_long))
)


# Ultimately, just group by site  (Abbreviation) since lat and long seem to have some kind of drift

kable(df_loc, caption = "Decision to group by site  (Abbreviation) since lat and long seem to have some kind of drift") %>% kable_styling()
...
# 
write.csv(CBCCounts, file.path(baci_home_dir, "data", "output", "Wauchope",
'BirdCounts.csv'), row.names = FALSE)
