...
## delete once upload is complete
# navigate one folder up
baci_home_dir <- normalizePath(file.path(getwd(), "..", "..",".."))
...
cat("Parent directory: ", dirname(getwd()), "\n")
```
...
avi_f <- AviList %>% filter(family %in% waterbird_families )

matched_families <- intersect(waterbird_families, unique(AviList$family))
unmatched_families <- setdiff(waterbird_families, unique(AviList$family))
cat('Unmatched waterbird families are: ', unmatched_families, "\n", "\n")
cat('Matched waterbird families are: ', matched_families, "\n")

# total waterbird species in AviList
unique_sp_avi <- avi_f %>% filter(taxon_rank == "species")
cat('Number of species = ', nrow(unique_sp_avi), "\n note: this excludes subspecies")
unique_sub_sp_avi <- avi_f %>% filter(taxon_rank == "subspecies")
cat("Number of subspecies = ", nrow(unique_sub_sp_avi))
...
### Step 2 - Filter CBC entries with "sp." -- will not find match to species 
CBCCounts_f <- CBCCounts %>% filter(!str_detect(SCI_NAME, 'sp.'))
...
### Step 3 - Keep CBC entries that have an exact genus species (and maybe subspecies) match with waterbirds avilist  
# save to new dataframe
# Note: logic here is to remove the shoe-ins, so that the edge cases can
# be more easily detected


### NOTE TS - see if these are exact matches (is subspecies getting passed?)
# keep exact match of genus species
CBC_exact_match <- CBCCounts_f %>% filter(SCI_NAME %in% unique(avi_f$scientific_name))
cat(nrow(CBC_exact_match), " rows that have exact match with avilist", "\n","\n" )

# remove rows with exact genus species match, and family not in list
# step a) assign family based on original AviList
# step b) remove based on filtered AviList
CBC_leftovers_with_fam <-CBC_exact_match %>% 
  mutate(family = AviList$family[match(SCI_NAME, AviList$scientific_name)]
)

CBC_keep_families <- CBC_exact_match_with_fam %>% filter(family %in% waterbird_families)


# further investigate edge cases here
CBCCounts_leftover <- CBCCounts_f %>% filter(!SCI_NAME %in% unique(avi_f$scientific_name))
cat(nrow(CBCCounts_leftover), "rows without exact match with avilist")

### Step 4 - Investigate edge cases
# Detect subspecies in Avilist
# Deal with / cases


# remove leftovers of excluded families
CBCCounts_leftover_with_fam<- CBCCounts_leftover %>% 
  mutate(family = AviList$family[match(SCI_NAME, AviList$scientific_name)]
) 



## are all leftovers then non-waterbird families?
## only subspecies aggregation left?


### Step 5 - Aggregate subspecies
