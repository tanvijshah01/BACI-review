...
# Sanity check - number of unique species in Avilist
# counting number of unique scientific names
avi_f_sp <- avi_f %>% filter(taxon_rank == "species")
ct_avi_sp <- length(unique(avi_f_sp$scientific_name))
cat("number of unique scientific names in AviList = ", ct_avi_sp, "\n","\n" )

# count number of unique waterbird species in CBC data
CBC_total_match_sp <- unique(CBC_total_match$SCI_NAME)
ct_CBC_sp <- length(unique(CBC_total_match$SCI_NAME))
cat("number of unique scientific names in CBC waterbirds= ", ct_CBC_sp)
